...
namespace utility {
  ...
  template<unsigned Size> struct fixed_string
  {
    ...
    {
      for (auto i = 0u; i < Size; ++i) { storage[i] = str[i]; }
    }
    ...
    constexpr friend auto operator<<(auto &os,
      const fixed_string &fs) -> decltype(auto)
    {
...
public:
  ...
  constexpr auto on(const events::assert_fail<TExpr> &event)
  {
    if (!__builtin_is_constant_evaluated())
    {
      ...
      os << event.file_name << ':' << event.line << ':' << "FAILED:" << '\"'
         << current_test.name << "\": " << event.expr;
    }
  }
  ...
  constexpr auto on(const events::summary &event)
  {
    if (!__builtin_is_constant_evaluated())
    {
      if (event.asserts[events::summary::FAILED]
          || event.tests[events::summary::FAILED])
      {
        os << "\nFAILED: ";
      } else
      {
      ...
      os << "tests: "
         << (event.tests[events::summary::PASSED]
              + event.tests[events::summary::FAILED])
         ...
         << "asserts: "
         << (event.asserts[events::summary::PASSED]
              + event.asserts[events::summary::FAILED])
...
template<class TOutputter, auto MaxDepth = 16u> struct reporter
{
  constexpr auto on(const events::test_begin<events::mode::run_time> &event)
  {
  ...
  {
    const auto result =
      summary.asserts[events::summary::FAILED] == asserts_failed[--current];
  ...
  {// non constexpr
    outputter.on(summary);
    if (summary.asserts[events::summary::FAILED]) { __builtin_abort(); }
  }
#endif
...
template<class TReporter> struct runner
{
  template<class Test>
  constexpr auto
    on(Test test, const char *file_name, int line, const char *name) -> bool
  {
    if (__builtin_is_constant_evaluated())
    {
      if constexpr (!type_traits::is_mutable_lambda_v<
                      decltype(&Test::operator())>)
      {
        test();
        return true;
      } else
    ...
    } else
    {
      ...
      {
        reporter.on(events::test_begin<events::mode::compile_time>{
          file_name, line, name });
        static_assert((test(), "[FAILED]"));
        reporter.on(events::test_end<events::mode::compile_time>{
          file_name, line, name });
      }
#endif
...
#if !defined(UT_COMPILE_TIME_ONLY)
      reporter.on(
        events::test_begin<events::mode::run_time>{ file_name, line, name });
      test();
      reporter.on(
        events::test_end<events::mode::run_time>{ file_name, line, name });
#endif
    }
    return true;
  }
...
template<class...> struct default_cfg
{
  struct
  {
    friend constexpr decltype(auto) operator<<([[maybe_unused]] auto &os,
...
inline constexpr struct
{
    ...
    int line = __builtin_LINE()) const
  {
    if constexpr (constexpr auto supported =
                    type_traits::is_same_v<bool, decltype(expr)>
                    || !requires { static_cast<bool>(expr); };
                  supported)
    {
      static_assert(
        !supported, "[ERROR] Expression required: expect(42_i == 42)");
    } else
    {
      bool result{};
      ...
      {
        detail::cfg(expr).reporter.on(
          events::assert_pass{ file_name, line, expr });
      } else
      {
        detail::cfg(expr).reporter.on(
          events::assert_fail{ file_name, line, expr });
      }
      return log{ result };
    }
  }
  ...
  struct fatal_expr
  {
    constexpr fatal_expr(auto expr,
      const char *file_name = __builtin_FILE(),
...
template<class T> [[nodiscard]] constexpr auto &mut(const T &t)
{
...
template<class TLhs, class TRhs> struct eq
{
  ...
  static_assert(type_traits::is_same_v<TLhs, TRhs>,
    "[ERROR] Comparision of different types is not allowed");
...
template<class T> struct eq<T, T>
{
  constexpr eq(const T &lhs, const T &rhs)
    : lhs{ lhs }, rhs{ rhs }, result{ lhs == rhs }
  {}
...
struct eq<T, T>
{
  ...
  struct epsilon
  {
    ...
    [[nodiscard]] constexpr explicit operator bool() const { return result; }
    constexpr friend auto operator<<(auto &os,
      const epsilon &expr) -> decltype(auto)
    {
      return (
        os << "(" << expr.lhs << " == " << expr.rhs << ")(" << expr.e << ")");
    }
    T lhs;
    T rhs;
    T e;
...
struct eq<TLhs, TRhs>
{
  static_assert(type_traits::is_same_v<decltype(utility::declval<TLhs>()[0]),
                  decltype(utility::declval<TLhs>()[0])>,
...
{
  short VALUE{};
  constexpr auto operator-() const { return _s(-VALUE); }
};
struct _i
{
  int VALUE{};
  constexpr auto operator-() const { return _i(-VALUE); }
};
struct _l
{
  long VALUE{};
  constexpr auto operator-() const { return _l(-VALUE); }
};
struct _ll
{
  long long VALUE{};
  constexpr auto operator-() const { return _ll(-VALUE); }
};
struct _u
{
  unsigned VALUE{};
...
{
  float VALUE{};
  constexpr auto operator-() const { return _f(-VALUE); }
};
struct _d
{
  double VALUE{};
  constexpr auto operator-() const { return _d(-VALUE); }
};
struct _ld
{
  long double VALUE{};
  constexpr auto operator-() const { return _ld(-VALUE); }
};
struct _i8
{
  __INT8_TYPE__ VALUE{};
  constexpr auto operator-() const { return _i8(-VALUE); }
};
struct _i16
{
  __INT16_TYPE__ VALUE{};
  constexpr auto operator-() const { return _i16(-VALUE); }
};
struct _i32
{
  __INT32_TYPE__ VALUE{};
  constexpr auto operator-() const { return _i32(-VALUE); }
};
struct _i64
{
  __INT64_TYPE__ VALUE{};
  constexpr auto operator-() const { return _i64(-VALUE); }
};
struct _u8
{
...
static_assert((
  [] {
    ...
    // ut::type_traits::is_mutable_lambda_v
    {
      ...
      static_assert(!ut::type_traits::is_mutable_lambda_v<
                    decltype(&decltype(l1)::operator())>);
      static_assert(!ut::type_traits::is_mutable_lambda_v<
                    decltype(&decltype(l2)::operator())>);
      static_assert(ut::type_traits::is_mutable_lambda_v<
        decltype(&decltype(l3)::operator())>);
      static_assert(ut::type_traits::is_mutable_lambda_v<
        decltype(&decltype(l4)::operator())>);
    }
    ...
    // ut::_
    {
      ...
      static_assert(
        sizeof("foo") == _string::view{ "foo", sizeof("foo") }.size());
