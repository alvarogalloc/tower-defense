...
//   sf::Vector2u grid_space_size = { 64, 64 })
// {
...
//     my_assert(ImGui::SFML::UpdateFontTexture(), "could not build font
//     atlas"); const auto middle_screen = sf::VideoMode::getDesktopMode().width
//     / 4; win.setPosition({ static_cast<int>(middle_screen), 0 });
//     win.setFramerateLimit(60);
//     win.setKeyRepeatEnabled(false);
//
//     my_assets manager{ path };
//     using namespace std::literals;
//     manager.set_loader<sf::Texture>(
//       [](sf::Texture *text, std::string_view path) {
//         if (!text->loadFromFile(path.data()))
//           throw std::runtime_error{ "Failed to load asset"s + path.data() };
//       });
//     manager.set_loader<sf::Music>([](sf::Music *music, std::string_view path)
//     {
//       if (!music->openFromFile(path.data()))
//         throw std::runtime_error{ "Failed to open music"s + path.data() };
//     });
//
//     // auto music =
//     auto *music = manager.get<sf::Music>("../bgmusic.mp3");
//     music->play();
...
//     const auto max_in_level =
//     level_manager.get_current_level().m_waves.size(); while (win.isOpen())
//     {
...
//               // position
//               projectile.direction = normalize(mouse_pos -
//               anim.getPosition()); projectile.speed = 500; projectile.damage
//               = 5; projectile.setSize({ 10, 10 });
...
public:
  virtual void on_start() override
  {
    m_curent_map = m_manager->get<Tilemap>(current_map);
  }
  virtual void on_update(float delta) override {}
  virtual void on_event(const sf::Event &e) override
  {
    ...
    {
      current_map = current_map == "1.tmx" ? "2.tmx" : "1.tmx";
      m_curent_map = m_manager->get<Tilemap>(current_map);
