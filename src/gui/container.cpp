  ...
  if (dir == direction::horizontal) {
    Vector2 widgets_size = std::accumulate(
        ...
        [this](auto acc, auto widget) {
          return acc +
                 std::visit([](auto el) { return el->get_size(); }, widget) +
                 Vector2{gap, gap};
        });
...
void container::draw() const { draw({0, 0}); }
...
void container::draw_box_model(Vector2 parent_end) const {
  ...
  const auto [w,h] =this->get_size();
  DrawRectangleLinesEx(
      {container_pos.x, container_pos.y, w, h},
      line_width, colors::red);
}
void container::draw_box_model() const { draw_box_model({0, 0}); }
