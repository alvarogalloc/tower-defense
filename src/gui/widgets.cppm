...
export namespace gui {
  template <typename T>
  using children_collection = std::vector<std::shared_ptr<T>>;
  ...
  struct node {
    ...
    {}
    Vector2 offset;
    children_collection<node> children;

    template <typename T, typename... Args>
      requires std::derived_from<T, node> || std::same_as<T, node>
    ...
    virtual void draw(Vector2 parent_pos)
    {
      Vector2 pos = {parent_pos.x + offset.x, parent_pos.y + offset.y};
  ...
  struct text : public node {
    text(std::string_view str, Color color, float font_size)
        : str(str)
        , color(color)
        , font_size(font_size)
    {}
  ...
  struct button : public text {
    ...
    Vector2 padding {};
    button(
      std::string_view str, Color color, float font_size, Vector2 padding,
      std::function<void(button&)>&& on_click)
        : text(str, color, font_size)
...
export namespace gui::v2 {
  ...
  struct button : node {
    ...
    {
      GuiButton(
        {parent_pos.x, parent_pos.y, get_size().x, get_size().y}, str.c_str());
    }
  };
  ...
  void container::draw_box_model() const
  {
    ...
    std::accumulate(
      ...
      [this](Vector2 acc, const child_t& child) {
        if (this->dir == direction::horizontal) {
          DrawRectangleLinesEx(
            {acc.x, acc.y, child->get_size().x, child->get_size().y}, 1, colors::blue);
