...
void update(entt::registry &db, float dt) {
  auto view = db.view<movement, health>();
  view.each([&](movement &player, health &player_health) {
    ...
    if (thrust.x != 0.0f || thrust.y != 0.0f) {
      thrust = Vector2Scale(Vector2Normalize(thrust),
      ...
      Vector2 deceleration = Vector2Scale(Vector2Normalize(player.velocity),
                                          -player.deceleration_rate);
  ...
  auto view2 = db.view<action, systems::player::gun, const Sound>();
  view2.each([&](entt::entity entity, action &action, gun &gun,
                 const Sound &sfx_shoot) {
    // get action
    ...
    switch (action) {
      case action::shoot: {
        ...
        gun.ammo -= 1;
        const auto &player = db.get<components::movement>(entity);
...
void draw(entt::registry &db) {
  auto v = db.view<movement, Texture>();
  v.each([&](entt::entity id, movement &player, Texture &player_texture) {
    ...
    const auto center = Vector2{dest.width / 2, dest.height / 2.f};
    ...
    if (db.all_of<systems::player::gun>(id)) {
      const auto &gun_info = db.get<systems::player::gun>(id);
