...
void *SDL_tex_loader(const char *path) { return IMG_LoadTexture(ren, path); }
...
void draw_polyline(double **points, double x, double y, int pointsc) {
  int i;
  for (i = 1; i < pointsc; i++) {
    SDL_RenderDrawLine(ren, x + points[i - 1][0], y + points[i - 1][1],
...
void draw_polygon(double **points, double x, double y, int pointsc) {
  ...
  if (pointsc > 2) {
    SDL_RenderDrawLine(ren, x + points[0][0], y + points[0][1],
...
void draw_objects(tmx_object_group *objgr) {
  ...
  while (head) {
    if (head->visible) {
      ...
      } else if (head->obj_type == OT_POINT) {
        // draw a point
        rect.x = head->x;
        rect.y = head->y;
        rect.w = 5;
...
void draw_tile(void *image, unsigned int sx, unsigned int sy, unsigned int sw,
  ...
  dest_rect.x = dx;
  dest_rect.y = dy;
  SDL_RenderCopyEx(ren, (SDL_Texture *)image, &src_rect, &dest_rect, 0, NULL,
                   0);
}
...
void draw_image_layer(tmx_image *image) {
  ...
  SDL_Texture *texture =
      (SDL_Texture *)image->resource_image; // Texture loaded by libTMX
...
int main(int argc, char **argv) {
  ...
  if (!(win = SDL_CreateWindow("SDL2 example", SDL_WINDOWPOS_UNDEFINED,
