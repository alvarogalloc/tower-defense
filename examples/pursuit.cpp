  ...
  public:
    ...
    void update(float delta)
    {
        ...
        const auto direction =
            Vector2Normalize(Vector2Subtract(next_position, current_position));
        current_position = Vector2Add(current_position,
                                      Vector2Scale(direction, speed * delta));
    }
    void draw() const
    {
        if (m_points.size() < 2)
