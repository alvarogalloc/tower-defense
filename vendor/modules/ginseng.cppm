...
namespace _detail {
  ...
  public:
    ...
    bool using_sdo() const { return numbits == word_size; }
  ...
  template<typename DB> struct database_traits
  {
    ...
    private:
      template<typename Com>
      static bool
        check(DB &db, ent_id eid, type_guid guid, component_tags::positive)
      {
      ...
      template<typename Com>
      static bool
        check(DB &db, ent_id eid, type_guid guid, component_tags::inverted)
      {
        using component = typename component_traits<Com>::component;
        if constexpr (std::is_same_v<Primary, component>)
        {
      ...
      template<typename Com>
      static bool check([[maybe_unused]] DB &db,
        [[maybe_unused]] ent_id eid,
      ...
      auto apply(DB &db, ent_id eid, com_id primary_cid, Visitor &&visitor)
      {
        if (key.check(db, eid))
        {
          return std::forward<Visitor>(visitor)(
            get_com<Components>(tag_t<Components>{},
              db,
              eid,
              primary_cid,
              get_guid<Components>(),
    ...
    private:
      ...
      static Com &get_com(component_tags::normal,
        DB &db,
        const ent_id &eid,
      ...
      template<typename Com, typename Primary>
      static Com get_com(component_tags::optional,
        DB &db,
        const ent_id &eid,
        const com_id &primary_cid,
        type_guid guid,
        primary<Primary>)
      {
        ...
        return get_com_optional<inner_component>(
          inner_category{}, db, eid, primary_cid, guid, primary<Primary>{});
      }

      template<typename Com, typename Primary>
      static optional<Com> get_com_optional(component_tags::normal,
        DB &db,
        const ent_id &eid,
        const com_id &primary_cid,
        type_guid guid,
      ...
      template<typename Com, typename Primary>
      static optional<Com> get_com_optional(component_tags::tagged,
        DB &db,
        const ent_id &eid,
      ...
      template<typename Com, typename Primary>
      static Com get_com(component_tags::unit,
        [[maybe_unused]] DB &db,
  ...
  protected:
    void set_count(size_type new_count) { count = new_count; }
  ...
  public:
    ...
    size_type capacity() const { return back_index; }
  ...
  public:
    ...
    bool operator==(const opaque_index &other) { return index == other.index; }
  ...
  public:
    ...
    void destroy_entity(const ent_id &eid)
    {
      ...
      if (entities[index].version != eid.version) { return; }

      for (dynamic_bitset::size_type i = 1;
           i < entities[index].components.size();
           ++i)
      {
    ...
    template<typename Com>
    auto get_component(
      ent_id eid) -> std::conditional_t<std::is_pointer_v<Com>, Com, Com &>
    {
      ...
      if constexpr (std::is_pointer_v<Com>)
      {
        ...
        if (entities[index].version != eid.version) { return nullptr; }
    ...
    template<typename Com> bool has_component(ent_id eid)
    {
      ...
      if (entities[index].version != eid.version) { return false; }
  ...
  private:
    ...
    {
      if (guid >= component_sets.size()) { return nullptr; }
    ...
    {
      auto guid = get_type_guid<Com>();
      if (component_sets.size() <= guid) { component_sets.resize(guid + 1); }
      auto &com_set = component_sets[guid];
      if (!com_set) { com_set = std::make_unique<component_set_impl<Com>>(); }
      auto com_set_impl = static_cast<component_set_impl<Com> *>(com_set.get());
      return *com_set_impl;
    }
