...
struct index_of<T, U, Ts...>
    : std::integral_constant<std::size_t, 1 + index_of_v<T, Ts...>>
{
};
...
template <typename T> type_guid get_type_guid()
{
...
/*! Tag component
 *
 * When a tag component is stored in an entity, only the fact that it exists
 * is recorded, no data is stored.
 */
...
/*! Require component
 *
 * When used as a visitor parameter, applies the matching logic for the
 * parameter, but does not load the component.
 */
template <typename T> struct require
{
};

/*! Deny component
 *
 * When used as a visitor parameter, entities that have the component will
 * fail to match.
 */
template <typename T> struct deny
{
};

/*! Optional component
 *
 * When used as a visitor parameter, loads the component if it exists, and
 * does not cause matching to fail.
 *
 ...
 *
 * When used as a visitor parameter, checks if the tag exists, but does not
 * cause matching to fail.
 */
template <typename T> class optional<tag<T>>
{
  public:
    optional() : tag(false)
    {
...
template <typename T> class optional<require<T>>
{
  public:
    static_assert(false_t<T>::value,
                  "Optional require parameters not allowed.");
};

template <typename T> class optional<deny<T>>
{
  public:
    static_assert(false_t<T>::value, "Optional deny parameters not allowed.");
};

template <typename T> class optional<optional<T>>
{
  public:
    static_assert(false_t<T>::value,
                  "Optional optional parameters not allowed.");
};
...
template <typename DB, typename Component> struct component_traits
{
...
template <typename DB> struct component_traits<DB, typename DB::ent_id>
{
...
template <typename T, typename... Ts> struct first
{
    using type = T;
};
...
template <typename T> struct primary
{
    using type = T;
};
...
template <typename DB, typename... Components> struct get_primary;
...
struct get_primary<DB, HeadCom, Components...>
{
    ...
    using type =
        std::conditional_t<std::is_same_v<category, component_tags::normal>,
                           primary<HeadCom>, get_primary_t<DB, Components...>>;
};

template <typename DB> struct get_primary<DB>
{
...
template <typename DB> struct database_traits
{
    using ent_id = typename DB::ent_id;
    using com_id = typename DB::com_id;

    template <typename C> using component_traits = component_traits<DB, C>;
    ...
    template <typename Primary, typename... Coms> class visitor_key;
    ...
    class visitor_key<primary<Primary>, Coms...>
    {
      public:
        ...
        bool check(DB &db, ent_id eid) const
        {
            return (
                check<Coms>(db, eid,
                            get_guid(index_of_v<com_t<Coms>, com_t<Coms>...>),
                            tag_t<Coms>{}) &&
                ...);
        }
      ...
      private:
        template <typename Com>
        static bool check(DB &db, ent_id eid, type_guid guid,
                          component_tags::positive)
        {
        ...
        template <typename Com>
        static bool check([[maybe_unused]] DB &db, [[maybe_unused]] ent_id eid,
                          [[maybe_unused]] type_guid guid, component_tags::meta)
        {
            return true;
        }
    ...
    template <typename... Components> struct visitor_traits_impl
    {
        using ent_id = typename DB::ent_id;
        using com_id = typename DB::com_id;
        ...
        template <typename T> type_guid get_guid() const
        {
        ...
        auto apply(DB &db, ent_id eid, com_id primary_cid, Visitor &&visitor)
        {
            if (key.check(db, eid))
            {
                return std::forward<Visitor>(visitor)(get_com<Components>(
                    tag_t<Components>{}, db, eid, primary_cid,
      ...
      private:
        ...
        static Com &get_com(component_tags::normal, DB &db, const ent_id &eid,
                            const com_id &primary_cid, type_guid guid,
                            primary<Primary>)
        {
        ...
        template <typename Com, typename Primary>
        static Com get_com(component_tags::optional, DB &db, const ent_id &eid,
                           const com_id &primary_cid, type_guid guid,
                           primary<Primary>)
        {
            ...
            return get_com_optional<inner_component>(inner_category{}, db, eid,
                                                     primary_cid, guid,
                                                     primary<Primary>{});
        }

        template <typename Com, typename Primary>
        static optional<Com> get_com_optional(component_tags::normal, DB &db,
                                              const ent_id &eid,
                                              const com_id &primary_cid,
                                              type_guid guid, primary<Primary>)
        {
            ...
            else
            {
                ...
                {
                    return optional<Com>(
                        db.template get_component<Com>(eid, guid));
                }
                else
                {
                    return optional<Com>();
        ...
        template <typename Com, typename Primary>
        static optional<Com> get_com_optional(
            component_tags::tagged, DB &db, const ent_id &eid,
            [[maybe_unused]] const com_id &primary_cid, type_guid guid,
            primary<Primary>)
        {
        ...
        template <typename Com, typename Primary>
        static const ent_id &get_com(component_tags::eid,
                                     [[maybe_unused]] DB &db, const ent_id &eid,
                                     [[maybe_unused]] const com_id &primary_cid,
                                     [[maybe_unused]] type_guid guid,
                                     primary<Primary>)
        {
            return eid;
        }
        ...
        template <typename Com, typename Primary>
        static Com get_com(component_tags::unit, [[maybe_unused]] DB &db,
                           [[maybe_unused]] const ent_id &eid,
                           [[maybe_unused]] const com_id &primary_cid,
                           [[maybe_unused]] type_guid guid, primary<Primary>)
        {
            return {};
        }
...
template <typename T> class component_set_impl final : public component_set
{
  public:
    ...
    size_type assign(size_type entid, T com)
    {
        ...
        if (index == back_index)
        {
            if (bucket == buckets.size())
            {
                ...
                comid_to_entid.resize(comid_to_entid.size() + bucket_size,
                                      null_id);
            }
...
template <typename Tag, typename Friend, typename Index> class opaque_index
{
  public:
...
class database
{
  public:
     ...
     */
    using com_id =
        opaque_index<struct com_id_tag, database, component_set::size_type>;
    ...
    void destroy_entity(const ent_id &eid)
    {
        ...
        for (dynamic_bitset::size_type i = 1;
             i < entities[index].components.size(); ++i)
        {
    ...
    {
        return entities[eid.index].version == eid.version &&
               entities[eid.index].components.get(0);
    }
     ...
     *
     * Otherwise, moves or copies the given component and adds it to the
     * entity.
     *
     ...
     */
    template <typename T> com_id add_component(const ent_id &eid, T &&com)
    {
     ...
     * @param com Tag value.
     */
    template <typename T> void add_component(ent_id eid, tag<T>)
    {
        auto index = eid.get_index();
    ...
    template <typename T> void add_component(ent_id eid, deny<T> com) = delete;

    template <typename T>
    void add_component(ent_id eid, optional<T> com) = delete;

    template <typename T> void add_component(ent_id eid, ent_id com) = delete;
     ...
     */
    template <typename Com> void remove_component(ent_id eid)
    {
        auto index = eid.get_index();
    ...
    /*! Get a component.
     *
     * If Com is a non-pointer type, returns a reference to the component
     * without performing safe checks for existence.
     *
     * Otherwise, if Com is a pointer type,
     * returns a pointer to the component of the pointed-to type that is
     * associated with the given entity.
     *
     * If the entity has no associated component of the given type, returns
     * nullptr.
     *
     ...
     *
     * @param eid ID of the entity.
     * @return Either a reference to the component, or a pointer to the
     * component, or nullptr.
     */
    template <typename Com>
    auto get_component(ent_id eid)
        -> std::conditional_t<std::is_pointer_v<Com>, Com, Com &>
    {
     ...
     *
     * Gets a reference to the component of the given type that has the
     * given ID.
     *
     * @warning
     * Behavior is undefined when no component is associated with the given
     * ID.
     *
     * @tparam Com Type of the component to get.
     *
     ...
     */
    template <typename Com> Com &get_component_by_id(com_id cid)
    {
        auto &com_set = *get_com_set<Com>();
        return com_set.get_com(cid);
    }
     ...
     */
    template <typename Com> bool has_component(ent_id eid)
    {
        auto index = eid.get_index();
    ...
    /*! Visit the Database.
     *
     * Visit the Entities in the Database that match the given function's
     * parameter types.
     *
     * The following parameter categories are accepted:
     *
     * - `T`, matches entities that have component `T`, and loads the
     * component.
     * - `tag<T>`, matches entities that have component `tag<T>`.
     * - `require<T>`, matches entities that have component `T`, but does
     * not load it.
     * - `optional<T>`, matches all entities, loads component `T` if it
     * exists.
     * - `deny<T>`, matches entities that do *not* match component `T`.
     * - `ent_id`, matches all entities, provides the `ent_id` of the
     * current entity.
     *
     * `T` and `optional<T>` parameters will refer to the entity's matching
     * component. `ent_id` parameters will contain the entity's `ent_id`.
     * Other parameters will be their default value.
     *
     * Entities that do not match all given parameter conditions will be
     * skipped.
     *
     * @warning Entities are visited in no particular order, so creating and
     * destroying entities or adding or removing components from within the
     * visitor could result in weird behavior.
     *
    ...
    template <typename Visitor> void visit(Visitor &&visitor)
    {
        ...
        return visit_helper(std::forward<Visitor>(visitor),
                            primary_component{});
    }
     ...
     */
    template <typename Com> component_set::size_type count() const
    {
     ...
     *
     * @warning This is not a valid pointer and relies on widespread
     * compiler-specific behavior. Do not ever dereference the pointer.
     * Entity version checking is not preserved through conversion to and
     * from pointers. Null pointers are a valid result.
     *
     * @param eid Entity ID to convert to a pointer.
     * @return A pointer which may be converted back into the same ID using
     * from_ptr(ptr).
     */
    auto to_ptr(const ent_id &eid) const -> void *
    {
        static_assert(sizeof(void *) >= sizeof(ent_id::index_type),
                      "Pointer conversion not possible");
        return reinterpret_cast<void *>(eid.get_index());
    }

    /*! Converts a void* to an ent_id. The pointer must have been returned
     * from to_ptr(eid).
     *
     * @warning The entity must not have been deleted. Version checking is
     * not applied.
     *
  ...
  private:
    ...
    template <typename Com> Com &get_component(ent_id eid, type_guid guid)
    {
    ...
    template <typename Com> Com &get_component_by_id(com_id cid, type_guid guid)
    {
        auto &com_set = *unsafe_get_com_set<Com>(guid);
        return com_set.get_com(cid);
    }

    template <typename Com> bool has_component(ent_id eid, type_guid guid)
    {
        auto &ent_coms = entities[eid.get_index()].components;
        return ent_coms.get(guid);
    }

    template <typename Com> component_set_impl<Com> *get_com_set()
    {
        return get_com_set<Com>(get_type_guid<Com>());
    }

    template <typename Com> const component_set_impl<Com> *get_com_set() const
    {
        return get_com_set<Com>(get_type_guid<Com>());
    }

    template <typename Com> component_set_impl<Com> *get_com_set(type_guid guid)
    {
    ...
    component_set_impl<Com> *unsafe_get_com_set(type_guid guid)
    {
        ...
        auto com_set_impl =
            static_cast<component_set_impl<Com> *>(com_set.get());
        return com_set_impl;
    }
    ...
    const component_set_impl<Com> *unsafe_get_com_set(type_guid guid) const
    {
        auto &com_set = component_sets[guid];
        auto com_set_impl =
            static_cast<const component_set_impl<Com> *>(com_set.get());
        return com_set_impl;
    }

    template <typename Com> component_set_impl<Com> &get_or_create_com_set()
    {
        auto guid = get_type_guid<Com>();
    ...
    void visit_helper(Visitor &&visitor, primary<Component>)
    {
        ...
        if (auto com_set_ptr =
                get_com_set<Component>(traits.template get_guid<Component>()))
        {
